// Copyright 2026 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.5
// 	protoc        v7.35.0
// source: google/ads/googleads/v25/common/local_services.proto

package common

import (
	enums "github.com/revealbot/google-ads-go/enums"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// A Local Services Document with read only accessible data.
type LocalServicesDocumentReadOnly struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// URL to access an already uploaded Local Services document.
	DocumentUrl   *string `protobuf:"bytes,1,opt,name=document_url,json=documentUrl,proto3,oneof" json:"document_url,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LocalServicesDocumentReadOnly) Reset() {
	*x = LocalServicesDocumentReadOnly{}
	mi := &file_common_local_services_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LocalServicesDocumentReadOnly) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LocalServicesDocumentReadOnly) ProtoMessage() {}

func (x *LocalServicesDocumentReadOnly) ProtoReflect() protoreflect.Message {
	mi := &file_common_local_services_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LocalServicesDocumentReadOnly.ProtoReflect.Descriptor instead.
func (*LocalServicesDocumentReadOnly) Descriptor() ([]byte, []int) {
	return file_common_local_services_proto_rawDescGZIP(), []int{0}
}

func (x *LocalServicesDocumentReadOnly) GetDocumentUrl() string {
	if x != nil && x.DocumentUrl != nil {
		return *x.DocumentUrl
	}
	return ""
}

// Phone number associated with the provider.
type LocalServicesPhoneNumber struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The phone number.
	PhoneNumber *string `protobuf:"bytes,1,opt,name=phone_number,json=phoneNumber,proto3,oneof" json:"phone_number,omitempty"`
	// Upper-case, two-letter codes as defined by ISO-3166.
	CountryCode *string `protobuf:"bytes,2,opt,name=country_code,json=countryCode,proto3,oneof" json:"country_code,omitempty"`
	// The type of the phone number.
	PhoneNumberType *enums.GlsPhoneNumberTypeEnum_GlsPhoneNumberType `protobuf:"varint,3,opt,name=phone_number_type,json=phoneNumberType,proto3,enum=google.ads.googleads.v25.enums.GlsPhoneNumberTypeEnum_GlsPhoneNumberType,oneof" json:"phone_number_type,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *LocalServicesPhoneNumber) Reset() {
	*x = LocalServicesPhoneNumber{}
	mi := &file_common_local_services_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LocalServicesPhoneNumber) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LocalServicesPhoneNumber) ProtoMessage() {}

func (x *LocalServicesPhoneNumber) ProtoReflect() protoreflect.Message {
	mi := &file_common_local_services_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LocalServicesPhoneNumber.ProtoReflect.Descriptor instead.
func (*LocalServicesPhoneNumber) Descriptor() ([]byte, []int) {
	return file_common_local_services_proto_rawDescGZIP(), []int{1}
}

func (x *LocalServicesPhoneNumber) GetPhoneNumber() string {
	if x != nil && x.PhoneNumber != nil {
		return *x.PhoneNumber
	}
	return ""
}

func (x *LocalServicesPhoneNumber) GetCountryCode() string {
	if x != nil && x.CountryCode != nil {
		return *x.CountryCode
	}
	return ""
}

func (x *LocalServicesPhoneNumber) GetPhoneNumberType() enums.GlsPhoneNumberTypeEnum_GlsPhoneNumberType {
	if x != nil && x.PhoneNumberType != nil {
		return *x.PhoneNumberType
	}
	return enums.GlsPhoneNumberTypeEnum_GlsPhoneNumberType(0)
}

var File_common_local_services_proto protoreflect.FileDescriptor

var file_common_local_services_proto_rawDesc = string([]byte{
	,
})

var (
	file_common_local_services_proto_rawDescOnce sync.Once
	file_common_local_services_proto_rawDescData []byte
)

func file_common_local_services_proto_rawDescGZIP() []byte {
	file_common_local_services_proto_rawDescOnce.Do(func() {
		file_common_local_services_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_common_local_services_proto_rawDesc), len(file_common_local_services_proto_rawDesc)))
	})
	return file_common_local_services_proto_rawDescData
}

var file_common_local_services_proto_msgTypes = make([]protoimpl.MessageInfo, 2)
var file_common_local_services_proto_goTypes = []any{
	(*LocalServicesDocumentReadOnly)(nil),                // 0: google.ads.googleads.v25.common.LocalServicesDocumentReadOnly
	(*LocalServicesPhoneNumber)(nil),                     // 1: google.ads.googleads.v25.common.LocalServicesPhoneNumber
	(enums.GlsPhoneNumberTypeEnum_GlsPhoneNumberType)(0), // 2: google.ads.googleads.v25.enums.GlsPhoneNumberTypeEnum.GlsPhoneNumberType
}
var file_common_local_services_proto_depIdxs = []int32{
	2, // 0: google.ads.googleads.v25.common.LocalServicesPhoneNumber.phone_number_type:type_name -> google.ads.googleads.v25.enums.GlsPhoneNumberTypeEnum.GlsPhoneNumberType
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_common_local_services_proto_init() }
func file_common_local_services_proto_init() {
	if File_common_local_services_proto != nil {
		return
	}
	file_common_local_services_proto_msgTypes[0].OneofWrappers = []any{}
	file_common_local_services_proto_msgTypes[1].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_common_local_services_proto_rawDesc), len(file_common_local_services_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   2,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_common_local_services_proto_goTypes,
		DependencyIndexes: file_common_local_services_proto_depIdxs,
		MessageInfos:      file_common_local_services_proto_msgTypes,
	}.Build()
	File_common_local_services_proto = out.File
	file_common_local_services_proto_goTypes = nil
	file_common_local_services_proto_depIdxs = nil
}
